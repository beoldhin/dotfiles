...
alias myshutdown="trizen -Syu; vim +PlugUpgrade +PlugClean! +PlugUpdate +qall; gem update; apm upgrade -c false; killgroup chrome; sudo bash -c 'gem update; sync; systemctl poweroff'"
alias myreboot="killgroup chrome; sync; sudo systemctl reboot"
...
alias myupgrade="trizen -Syu && sync"
alias myupgradeapm="apm upgrade -c false && sync"
alias myupgradegem="sudo gem update"
alias myupgradevim="vim +PlugUpgrade +PlugClean! +PlugUpdate +qall && sync"
alias myupgradecheck="sudo apt list --upgradeable && npm outdated -g --depth=0"
# Problems with npm updating: disabling for now
# alias myupgradeall="trizen -Syu; vim +PlugUpgrade +PlugClean! +PlugUpdate +qall; sudo gem update; gem update; apm upgrade -c false; sudo npm update -g; sync"
alias myupgradeall="trizen -Syu; vim +PlugUpgrade +PlugClean! +PlugUpdate +qall; sudo gem update; gem update; apm upgrade -c false; sync"

# Wed development definitions
alias mymongostart="sudo mkdir -p /var/log/mongodb && sudo chown mongodb:mongodb /var/log/mongodb && sudo systemctl start mongod && systemctl status mongod"
alias mymongostop="sudo systemctl stop mongod && systemctl status mongod"
